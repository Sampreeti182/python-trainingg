...
MYSQL_USER = "root"
MYSQL_PASSWORD = "Chillichicken@1802"  # <- updated to your password
...
def load_dataframe():
    conn = connect_db()
    try:
        # Ensure you select from the TABLE, not the database
...
def run_analytics(df: pd.DataFrame):
    ...
    print(df[df["days_left"].between(0, 7)][
              ["sub_id", "customer_name", "plan_type", "expiry_date", "days_left", "status"]])
...
def export_csv(df: pd.DataFrame, filename: str = "subscription_report.csv"):
    ...
    out = df.copy()
    # Convert any datetime-like objects to string for CSV
    ...
    # created_at already formatted as string earlier
